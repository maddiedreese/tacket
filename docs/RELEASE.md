...
The GitHub Actions release workflow builds `Tacket.dmg`, `tacket-chrome-extension.zip`, and `SHA256SUMS` on `v*` tags or manual dispatch. If signing and notarization secrets are configured, the workflow imports the Developer ID certificate, signs the app, packages the DMG, notarizes it, verifies the final artifacts, uploads an Actions artifact, and publishes a GitHub Release for tags.
